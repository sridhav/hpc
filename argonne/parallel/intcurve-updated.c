...
double simpson( double xmin, double xmax, int rank, int nranks, long int samples)
{
    ...
    for(i=1;i<localsamples;i=i+2) lsect=( 4 * f(lxmin+(i*xoffset)))+lsect;
    for(i=2;i<localsamples-1;i=i+2) lsect=( 2 * f(lxmin+(i*xoffset)))+lsect;
...
int main( int argc, char *argv[] )
{
    ...
    if (rank != 0)
    { 
        MPI_Isend(&ldfs,1,MPI_DOUBLE,0,rank,MPI_COMM_WORLD,&request[rank]);
        MPI_Isend(&lsimpson,1,MPI_DOUBLE,0,rank+nranks,MPI_COMM_WORLD,&request[rank+nranks]);
        MPI_Wait(&request[rank],&status);
        MPI_Wait(&request[rank+nranks],&status);
    }
    ...
    int *ranks;
    double *ldfs_list;
    double *lsimpson_list;
    if (rank == 0) {
        ranks = malloc(sizeof(int)*nranks);
        ldfs_list = malloc(sizeof(double)*nranks);
        lsimpson_list = malloc(sizeof(double)*nranks);
     }
   
    MPI_Gather(&rank, 1, MPI_INT, ranks, 1, MPI_INT,0, MPI_COMM_WORLD);
    MPI_Gather(&ldfs, 1, MPI_DOUBLE, ldfs_list, 1, MPI_DOUBLE, 0, MPI_COMM_WORLD);
    MPI_Gather(&lsimpson, 1, MPI_DOUBLE, lsimpson_list, 1, MPI_DOUBLE, 0, MPI_COMM_WORLD);
    
    if (rank==0) {
        for (k=0; k<nranks; k++) {
            printf("rank %3d deflt: % 26.6f simpson: % 26.6f\n",ranks[k],ldfs_list[k],lsimpson_list[k]);
        }
    }
    ...
    if (rank == 0)
    {
        ...
        for (k=1; k<nranks; k++)
        {       
            MPI_Irecv(&ldfs,1,MPI_DOUBLE,k,k,MPI_COMM_WORLD,&request[k]);
            MPI_Wait(&request[k],&status);
            fdfs=ldfs+fdfs;
            results[0].solver="default";
            results[0].area=fdfs;
            results[0].size=2;
            MPI_Irecv(&lsimpson,1,MPI_DOUBLE,k,k+nranks,MPI_COMM_WORLD,&request[k+nranks]);
            MPI_Wait(&request[k+nranks],&status);
            fsimpson=lsimpson+fsimpson;
